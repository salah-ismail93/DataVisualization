<template>
  <div class="bg-white px-6 lg:px-8 text-center mt-8">
    <div class="mx-auto max-w-5xl">
      ...
      <div>
        <label for="selectStateStack">Select State:</label>
        <select id="selectStateStack"></select>
      </div>
...
export default {
  ...
  mounted() {
    ...
    d3.csv("/stacked_bar_chart.csv").then(function (allData) {
      ...
      const selectStateStack = d3.select('#selectStateStack');
      selectStateStack
        .selectAll('option')
        .data(states)
        .enter()
      ...
      selectStateStack.on('change', function () {
    ...
    function drawBarChart(allData, selectedState, biggestCitiesInState) {
      ...
      // Add X axis
      var maxCount = d3.max(stackedData[stackedData.length - 1], d => d[1]);
      ...
      // Create a tooltip
      // ----------------
      var tooltip2 = d3.select("#A1chart3")
        .append("div")
        .attr("class", "tooltip")
      ...
      d3.select("#chart_3_legend svg").remove();

      var legend = d3.select("#chart_3_legend")
        .append("svg")
        .attr('width', 300)
        .attr('height', 200)
        .append('g')
